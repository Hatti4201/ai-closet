...
export const clothingApi = {
  ...
  getAll: (filters?: ClothingFilters): Promise<{ items: ClothingItem[] }> => {
    ...
    return api.get<{ items: ClothingItem[] }>(`/clothing?${params.toString()}`).then((r) => r.data);
  },

  getOne: (id: string): Promise<ClothingItem> =>
    api.get<ClothingItem>(`/clothing/${id}`).then((r) => r.data),
